...
class Tcp {
    ...
    public function read() {
        ...
        call_user_func( $o_yaw_instance->onMessage, $this, $a_http_decode_content );
    }

    /*
     * @desc : 向connection中写入内容
     * */
    public function write( $s_send_content ) {
    ...
    public function send( $s_send_content ) {
        //echo "tcp connection send".PHP_EOL;
        Core::$o_event_loop->add( $this->r_connection_socket, EventInterface::EV_WRITE, array( $this, "write" ) );
        $this->write( $s_send_content );
        //sleep( 5 );
        Core::$o_event_loop->del( $this->r_connection_socket, EventInterface::EV_ALL );
        //sleep( 10000 );
    }
